...
const fragmentShader = `
  ...
  void main() {
    ...
    if (uMouseActive > 0.01) {
      ...
      float ripple = sin(dist * uWaveFrequency * 5.0 - time * uWaveSpeed) * uWaveAmplitude * 0.05 * mouseInfluence * uMouseActive;
    ...
    float revealAmount = 1.0 - smoothstep(uRevealRadius * (1.0 - uRevealSoftness), uRevealRadius, revealDist);
...
function ImagePlane({ src, aspectRatio, revealRadius, revealSoftness, pixelSize, waveSpeed, waveFrequency, waveAmplitude, mouseRadius, isMouseInCanvas }) {
  ...
  useFrame((state) => {
    ...
    mouseActiveRef.current += ((isMouseInCanvas ? 1 : 0) - mouseActiveRef.current) * 0.06;
